...
static ssize_t sample_read(struct file *filp,char __user *buff,size_t count, loff_t *offp)
{
    ...
    wait_event_interruptible(_wq,CIRC_CNT(cbuf.head,cbuf.tail,SIZE)>0); // We are waiting For the Data available to us, otherwisw it will go on Sleep();
...
static int __init sample_init(void)
{
    ...
    init_waitqueue_head(&_wq); // Initialize Wait Queue
